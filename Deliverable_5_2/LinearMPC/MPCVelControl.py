...
class MPCVelControl:
    ...
    def new_controller(self, rocket: Rocket, Ts: float, H: float, x_target: np.ndarray = None, use_terminal_set: bool = False) -> None:
        self.xs, self.us = rocket.trim()
        A, B = rocket.linearize(self.xs, self.us)

        self.mpc_x = MPCControl_xvel(A, B, self.xs, self.us, Ts, H, x_target, use_terminal_set)
        self.mpc_y = MPCControl_yvel(A, B, self.xs, self.us, Ts, H, x_target, use_terminal_set)
        self.mpc_z = MPCControl_zvel(A, B, self.xs, self.us, Ts, H, x_target, use_terminal_set)
        self.mpc_roll = MPCControl_roll(A, B, self.xs, self.us, Ts, H, x_target, use_terminal_set)
    ...
    def estimate_parameters(self, x_data: np.ndarray, u_data: np.ndarray) -> None:
        # Extract the relevant data for z velocity controller
        z_x_data = x_data[self.mpc_z.x_ids, 0]
        z_u_data = u_data[self.mpc_z.u_ids]

        # Store the data in the mpc_z controller
        self.mpc_z.store_data(z_x_data, z_u_data)
        return
